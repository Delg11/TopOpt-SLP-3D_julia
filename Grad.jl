...
function Grad(str, u, p, emin, k, x, Dofs, W, w, opfilter, mr, strDens)
    ...
    if !mr.op
        Fgrad0 = zeros(str.nelem)
        for i in str.freeDensG
            uel = u[Int.(Dofs[i, :])] # Nodal displacements of the finite element i
            Fgrad0[i] = -p * (x[i]^(p - 1)) * (str.E - emin) * (uel' * k * uel)
        end
    else
        # Multiresolution
        Fgrad0 = zeros(strDens.nelem)
        nely = strDens.nely
        ...
        for i in str.freeDensG
            ...
            if !mr.interp
                # Using the original displacements to compute the gradient
                for i2 = 1:length(v)
                    kel = k[i2]  # Stiffness integrand for the density element
                    Fgrad0[v[i2]] =
                        -p * (x[v[i2]]^(p - 1)) * (str.E - emin) * (uel' * kel * uel)
                end
            else
                # Using interpolated displacements to compute the gradient
                ud = mr.Idisp * uel  # Interpolated displacements of all density elements inside the finite element i
                for i2 = 1:length(v)
                    ueli = ud[mr.Ldofs[i2, :]]  # Interpolated displacements of a single density element
                    kel = mr.kd  # Stiffness integrand for the density element
                    Fgrad0[v[i2]] =
                        -p * (x[v[i2]]^(p - 1)) * (str.E - emin) * (ueli' * kel * ueli)
                end
            end
        end
    end
