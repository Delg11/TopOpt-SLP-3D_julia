# Arquivo: ElemNeighborhoodMR.jl
module ElemNeighborhoodMRModule

export ElemNeighborhoodMR

function ElemNeighborhoodMR(strDens, strDsgn, rmin, mr, opfilter)
    ...
    iW = zeros(Int, strDens.nelem)
    jW = zeros(Int, strDens.nelem)
    sW = zeros(strDens.nelem)
    ...
    for k1 = 1:strDens.nelz
        ...
        for i1 = 1:strDens.nelx
            ...
            for j1 = 1:strDens.nely
                ...
                d2 = repeat(dx2, length(dy2)) .+ repeat(dy2', length(dx2))
                d2 = repeat(dz2, length(d2)) .+ repeat(d2', length(dz2))

                e21 = (k2 .- 1) * nelxyDsgn
                e22 = (i2 .- 1) * strDsgn.nely
                nb = repeat(e21, length(e22)) .+ repeat(e22', length(e21))
                nb = repeat(nb, length(j2)) .+ repeat(j2', length(nb))

                dind = findall(d -> d <= rmin2, d2)
                d = sqrt.(d2[dind])
                ld = ind + length(d)
