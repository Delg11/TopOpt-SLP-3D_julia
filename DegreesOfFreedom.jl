...
module DegreesOfFreedomModule

export DegreesOfFreedom

function DegreesOfFreedom(str, elem)
    ...
    if (elem.type == 1 || elemDeg == 1) # Lagrange elements
        ...
        nodes[:, 1] = vec(aux)  # 'vec' é usado para transformar a matriz em vetor (equivalente ao `(:)` do MATLAB)
        for k2 = 2:(elemDeg+1)
            nodes[:, k2] = nodes[:, 1] .+ (k2 - 1)
        end

        nodes = vec(nodes)  # 'vec' para achatar a matriz em um vetor
        dofs = [3 .* nodes .- 2; 3 .* nodes .- 1; 3 .* nodes]  # Usar broadcasting (.* e .-) para operações elemento a elemento
        dofs = sort(vec(dofs))  # 'vec' para achatar e 'sort' para ordenar
        ...
        for i = 1:str.nelem
            ...
            j += elemDeg
            if mod(j, str.ny) == 0
                j += 1 + (elemDeg - 1) * str.ny
                if mod(j, nxny) == nxnymnely
                    j += str.ny + (elemDeg - 1) * nxny
                    dofs .+= 3 * (elemDeg + 1 + elemDeg * str.ny + (elemDeg - 1) * nxny)
                else
                    dofs .+= 3 * (elemDeg + 1 + (elemDeg - 1) * str.ny)
                end
            else
                dofs .+= 3 * elemDeg
            end
        ...
        for el = 1:str.nelem
            ...
            for i = 1:n3
                ...
                v3 =
                    vt[1] +
                    (
                        nxy1 - ((ex - 1) * (ny1 + n3 * ny2)) +
                        (ex - 1) * ny2 +
                        (ey - 1) * (1 - elemDeg)
                    ) +
                    (i - 1) * nxy2 # from the vertex vt(1) go in the z-direction to the node v3
            ...
            nodes = vcat(vt, edx, edy, edz) # indexes of all nodes of the element 
            dofs = hcat(3 * nodes .- 2, 3 * nodes .- 1, 3 * nodes) # degrees of freedom of the element
            Dofs[el, :] = sort(dofs)
            ...
            if ey == str.nely
                ...
                if ex == str.nelx
                    ex = 1
                    ez += 1
                    j += elemDeg + 1 + n3 * ny2 + ny1 + n3 * nxy2
                else
                    ex += 1
                    j += elemDeg + 1 + n3 * ny2
                end
