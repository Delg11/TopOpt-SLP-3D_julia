...
module StartMRModule

export StarMR

# Definindo as structs para strDsgn
struct GridData
    l::Float64
    h::Float64
    w::Float64
    ...
    E::Float64
    nu::Float64
    fdv::Vector{Float64}  # First design variable inside each element
    fde::Vector{Float64}  # First density element inside each element
end
...
function StartMR(
    ...
    if !isempty(str.fixedDens)
        fixDens = zeros(Int, strDsgn.nelem)
        fixVal = zeros(Float64, strDsgn.nelem)
                ...
                for elx = ex0:exf
                    for ely = ey0:eyf
                        el =
                            ely +
                            (elx - 1) * strDsgn.nely +
                            (elz - 1) * strDsgn.nelx * strDsgn.nely
                        fixDens[el] = 1
        ...
        if mr.n != mr.d
            ...
            fixVal = zeros(Float64, strDens.nelem)
                    ...
                    for elx = ex0:exf
                        for ely = ey0:eyf
                            el =
                                ely +
                                (elx - 1) * strDens.nely +
                                (elz - 1) * strDens.nelx * strDens.nely
                            fixDens[el] = 1
                            if str.fix[i, 7] == 1  # solid region
                                fixVal[el] = 1.0
                            end
        ...
        strDsgn.freeDens = collect(1:strDsgn.nelem)
        strDsgn.fixedDens = []
        strDsgn.fixedDensVal = []
        strDens.freeDens = collect(1:strDens.nelem)
        strDens.fixedDens = []
    ...
    if mr.x0  # Solve the problem on the coarse grid and use the solution as initial design variables for the problem on the fine grid
        ...
        slp.tolG *= 100
        println(
            "Solving the problem on the coarse grid to obtain the initial design variables for the problem on the fine grid.",
        )

        _, xstarDsgn, _, _, _, _, _, _, _, _, _, _, _, _ = StartStrSLP(
            str,
            volfrac,
            p,
            p123,
        ...
        slp.tolS = ts
        slp.tolG = tg
        x0 = zeros(Float64, strDsgn.nelem)
        nely = strDsgn.nely
        ...
        mr.op = true
        println("Solving the problem on the fine grid.")
    else
        x0 = volfrac * ones(Float64, strDsgn.nelem)

        if !isempty(strDsgn.fixedDens)  # Assign fixed design variables
            x0[strDsgn.fixedDens] .= strDsgn.fixedDensVal
            velemDsgn =
                ((str.l * str.h * str.w) / strDsgn.nelem) * ones(Float64, strDsgn.nelem)  # Volume of the elements on the design variables grid
            Vfix = velemDsgn[strDsgn.fixedDens]' * x0[strDsgn.fixedDens]  # Volume occupied by fixed elements
            x0[strDsgn.freeDens] .=
                (volfrac * (str.l * str.h * str.w) - Vfix) /
                sum(velemDsgn[strDsgn.freeDens])  # Redistributing initial design variables of the free elements
        end
    end
