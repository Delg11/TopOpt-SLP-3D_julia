...
include("SetupGMG.jl")
include("SolveMG.jl")
include("NodalDisp.jl")
include("Grad.jl")
include("ApplyFilter.jl")

# Comandos using no topo
using .GlobalStiffMatrixModule  # O ponto indica um módulo local
using .SetupGMGModule  # O ponto indica um módulo local
using .ApplyFilterModule  # O ponto indica um módulo local
using .SolveMGModule  # O ponto indica um módulo local
using .NodalDispModule  # O ponto indica um módulo local
...
# Definição de um TimerSTR mutável
mutable struct TimerSTR
    elapsed_time::Float64  # Tempo total acumulado
    start_time::Float64    # Tempo de início para a medida atual
    active::Bool           # Indica se o timerSTR está ativo
...
function tic(timerSTR::TimerSTR)
    timerSTR.start_time = time()   # Captura o tempo atual
    timerSTR.active = true          # Indica que o timerSTR está ativo
end

function toc(timerSTR::TimerSTR)
    if timerSTR.active
        elapsed = time() - timerSTR.start_time  # Calcula o tempo decorrido
        timerSTR.elapsed_time += elapsed         # Acumula o tempo total
        return elapsed                        # Retorna o tempo decorrido
    else
        error("TimerSTR is not active. Please call 'tic' before 'toc'.")
    end
end

function get_elapsed(timerSTR::TimerSTR)
    return timerSTR.elapsed_time  # Método para acessar o tempo total acumulado
end
...
function StrSLP(
    ...
    # ---------- #
    # Some initial values
    timerSTR = TimerSTR()  # Inicializa o timer
    ...
    t.setupK += toc(timerSTR)
    u = zeros(3 * Int(str.nnodes))
    P = Vector{Any} # Prolongation matrices
    ...
    if mr.op && mr.interp  # Using the interpolated displacements on the density grid to calculate the objective function
        ...
        Fold = dot(vec(str.f), vec(u[str.freedofs]))
    end
    ...
    # SLP algorithm
    while (
        (countG < slp.maxcount || countF < slp.maxcount) &&  # Check gradient and function count limits
        (countS < slp.maxcount) &&  # Check step count limit
        (iter < slp.maxiter)  # Check iteration limit
    )
        it = iter + 1  # Current iteration
        # Create or update the JuMP model
        # model = Model(HiGHS.Optimizer)
        model = Model(optimizer_with_attributes(() -> Gurobi.Optimizer(GUROBI_ENV)))
        # set_optimizer_attribute(model, "output_flag", false) #HiGHS
        # set_optimizer_attribute(model, "time_limit", 1.0)
        set_optimizer_attribute(model, "OutputFlag", false)  # Disable Gurobi output (same as "output_flag" for HiGHS)
        set_optimizer_attribute(model, "TimeLimit", 1.0)  # Set the time limit for Gurobi solver
        # Define the decision variables with bounds
        @variable(model, s[i = 1:nelem], lower_bound = sL[i], upper_bound = sU[i])

        # Define the objective function
        @objective(model, Min, sum(Fgrad[i] * s[i] for i in 1:nelem))

        # Add inequality or equality constraints based on the volineq condition
        if volineq
            @constraint(model,c1, A * s .<= -b)
        else
            @constraint(model,c2, A * s .== -b)
        end
        tic(timerSTR)
        # Solve the LP model
        optimize!(model)
        ...
        flag = termination_status(model)
        if flag == MOI.OPTIMAL
            fval = objective_value(model)
            lpit[ns] = simplex_iterations(model)>0 ? simplex_iterations(model) : 0
            s = value.(s)[1:nelem] # Get only the relevant elements
        end    
        if flag != MOI.OPTIMAL
            # If the first LP problem is not optimal, redefine A and c for the new problem
            # Modify A and redefine c for the new problem
            A2 = hcat(A, -1, 1)
            c = [zeros(nelem); 1; 1]
            ...
            # Update the model with new variables and constraints
            @variable(model, s2[i = 1:(nelem+2)], lower_bound = sL2[i], upper_bound = sU2[i])
            @objective(model, Min, sum(c[i] * s2[i] for i in 1:length(c)))

            if volineq
                @constraint(model, A2 * s2 .<= -b)
            else
                @constraint(model, A2 * s2 .== -b)
            end

            # Solve the modified LP model
            optimize!(model)

            # Extract relevant results from the modified model
            s = value.(s2)[1:nelem]  # Get only the relevant elements
            fval = dot(Fgrad, s)  # Calculate the function value
            lpit[ns] = barrier_iterations(model)>0 ? barrier_iterations(model) : 0  # Store the number of iterations
        elseif lpit[ns] == 0
            # If no iterations were performed, set s to zero
            s = zeros(nelem)
        end

        # Timing the LP solving
        t.LP += toc(timerSTR)  # Update total time for LP

        # Calculate the step norm
        snorm = norm(s, Inf)

        # Save some values and update x
        xold = x  # Store the old x value
        x = x .+ s  # Update x with the step
        bold = b  # Store the old b value
        deltaold = delta  # Store the old delta value
...
# Supondo que predopt, predfsb, aredopt, aredfsb já estão definidos no seu código

# Função auxiliar para exibir tipo e valor
function debug_print(variable_name, value)
    println("$variable_name - Tipo: $(typeof(value)), Valor: $value")
end
        ...
        if all(predopt .> 0.5 * predfsb)
            thetaSup = 1
        else
            thetaSup = norm(predfsb .- predopt)==0 ? Inf : (0.5 * predfsb) / (predfsb .- predopt)
            # thetaSup = (0.5 * predfsb) / (predfsb .- predopt)
        end
        theta = min(thetaLarge, thetaSup[1][1])
        theta = min(theta, thetaMax)
        theta2 = theta1
        theta1 = theta

        # Predicted reduction of the merit function
        pred = (theta*predopt.+(1-theta)*predfsb)[1][1]

        # Actual reduction of the merit function
        ared = (theta*aredopt.+(1-theta)*aredfsb)[1][1]
        ...
        if ared < slp.eta .* pred  # Rejected
            ...
            if volineq
                Lgrad = Fgrad[freeDsgn]' .- dual.(c1[1]) * A[freeDsgn]'
            else
                Lgrad = Fgrad[freeDsgn]' .- dual.(c2[1]) * A[freeDsgn]'
            end
        ...
        @printf(
            "It: %3d | F: %+e | Ared: %+e | Pred: %+e | snorm: %f | delta: %f | gpnorm: %f | flagLP: %s | itLP: %3d | pcgit: %4d \n",
            it,
            F[1],
            ared,
            pred,
            ...
            deltaold,
            gpnorm,
            string(flag),  # Convertendo flag para string
            lpit[ns-1],
            pcgit[ns]
        )
    ...
    if prj.op
        ...
        if volineq
            Lgrad = Fgrad[freeDsgn]' .- dual.(c1[1]) .* A'
        else
            Lgrad = Fgrad[freeDsgn]' .- dual.(c2[1]) .* A'
        end
    ...
    return xstar, xstarDsgn, u, opstop, iter, itrej, F, vol, ns, pcgit, lpit, t, xhist
end
