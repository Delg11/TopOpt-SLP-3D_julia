# Arquivo: Prolongation.jl
module ProlongationModule

export Prolongation

using SparseArrays

function Prolongation(nxf, nyf, nzf, nxc, nyc, nzc, nc, freedofsf)
    # Prolongation obtains the prolongation matrix for geometric multigrid.
    # INPUT:
    #   nxf - number of fine nodes in the length(x) direction.
    ...
    iP = zeros(Int, maxind)
    jP = zeros(Int, maxind)
    sP = zeros(Float64, maxind)
    freedofsc = zeros(Int, 3 * nc)
    aux = zeros(Int, 3 * nc)
    ind = 0
    ...
    for zc = 1:nzc
        for xc = 1:nxc
            for yc = 1:nyc
                ...
                xf, yf, zf = 2 * xc - 1, 2 * yc - 1, 2 * zc - 1    # coordinates of the node on the fine grid
    ...
    P = sparse(iP[1:ind], jP[1:ind], sP[1:ind])  # Construct the sparse matrix
    freedofsc = freedofsc[findall(in(freedofsf), aux)]
    P = P[vec(freedofsf), vec(freedofsc)]  # Extract relevant rows and columns from the prolongation matrix
