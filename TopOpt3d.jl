...
mutable struct Material
    ...
    freedofs::Union{Vector{Int64},Nothing}
    loads::Union{Matrix{Float64},Nothing}
    f::Union{Vector{Float64},Nothing}
    fix::Union{Vector{Int},Nothing}
    freeDens::Union{Vector{Int64},Nothing}
    fixedDens::Union{Vector{Int64},Nothing}
    fixedDensVal::Union{Vector{Float64},Nothing}
    freeDensG::Union{Vector{Int64},Nothing}
...
mutable struct MaterialDens
    ...
    freeDens::Union{Matrix{Int},Vector{Int64},Nothing}
    fixedDens::Union{Matrix{Int},Vector{Int64},Nothing}
    fixedDensVal::Union{Matrix{Int},Vector{Float64},Nothing}
end
...
function load(file_name::String)
    ...
    if haskey(vars, "str")
        ...
        data = Dict{Symbol, Union{Float64, Matrix{Float64}, Vector{Float64}, Nothing}}()
        ...
        # Função auxiliar para aplicar `vec` apenas se o valor não for `nothing`
        function safe_vec(value)
            value === nothing ? nothing : vec(value)
        end
        ...
        material = Material(
            ...
            safe_vec(get(data, :freedofs, nothing)),
            get(data, :loads, nothing),
            safe_vec(get(data, :f, nothing)),
            safe_vec(get(data, :fix, nothing)),
            safe_vec(get(data, :freeDens, nothing)),
            safe_vec(get(data, :fixedDens, nothing)),
            safe_vec(get(data, :fixedDensVal, nothing)),
            safe_vec(get(data, :freeDensG, nothing)),
        )
...
symxy = false; # Defines if the problem domain has a symmetry with respect to the xy plane (true for cb, mbb, tt and hc problems)
symyz = false; # Defines if the problem domain has a symmetry with respect to the yz plane (true for mbb, tt and hc problems)
